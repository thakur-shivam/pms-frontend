...
const MasterTaskStatus = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (name: string) => api.post('/task-statuses/create', { status_name: name }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['taskStatuses'] });
      setNewStatusName('');
      toast.success('Task status created successfully');
    },
    onError: () => toast.error('Failed to create task status'),
  });

  const updateMutation = useMutation({
    mutationFn: (status: TaskStatus) => 
      api.put(`/task-statuses/update/${status.id}`, { status_name: status.status_name }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['taskStatuses'] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/task-statuses/delete/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['taskStatuses'] });
      toast.success('Task status deleted successfully');
    },
    onError: () => toast.error('Failed to delete task status'),
  });
  ...
  return (
    <div>
      ...
      <Table
        data={statuses || []}
        columns={columns as TableColumn<TaskStatus>[]}
        itemsPerPage={5}
      />
...
export default MasterTaskStatus;